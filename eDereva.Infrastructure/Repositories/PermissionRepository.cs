    ...
    : IPermissionRepository
{
    public async Task<Permission> GetByIdAsync(int permissionId)
    {
        ...
        {
            logger.LogInformation("Permission with ID {PermissionId} not found in cache. Fetching from database.",
                permissionId);
            return await context.Permissions
                .FirstOrDefaultAsync(p => p.Id == permissionId, entry) ?? new Permission();
        });
    ...
    public async Task<List<Permission>> GetAllAsync()
    {
        ...
        var permissions = await hybridCache.GetOrCreateAsync<List<Permission>>(cacheKey, async entry =>
        {
            ...
            return await context.Permissions.ToListAsync(entry);
        });
