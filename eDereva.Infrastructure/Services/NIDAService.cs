        ...
        public async Task<UserDto?> LoadUserDataAsync(string NIN)
        {
            ...
            return await hybridCache.GetOrCreateAsync<UserDto>(cacheKey, async (entry) =>
            {
                try
                {
                    ...
                    {
                        logger.LogError("Failed to retrieve user data from NIDA API. Status code: {StatusCode}, Reason: {ReasonPhrase}", response.StatusCode, response.ReasonPhrase);
                        return null!;
                    }
                    ...
                    logger.LogInformation("No result found in NIDA response for NIN: {NIN}. Error: {Error}", NIN, (string)error);
                    return null!;
